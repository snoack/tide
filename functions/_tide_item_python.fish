function _tide_item_python
    if test -n "$VIRTUAL_ENV"
        ...
        if test "$dir" = virtualenvs
            ...
            _tide_print_language_item python $tide_python_icon "$v ($base)"
        else if contains -- "$base" virtualenv venv .venv env # avoid generic names
            _tide_print_language_item python $tide_python_icon "$v ($dir)"
        else
            _tide_print_language_item python $tide_python_icon "$v ($base)"
        end
        ...
        _tide_print_language_item python $tide_python_icon $v
    end
end
